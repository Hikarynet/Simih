		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD BOT SIMIH *',
					benned: 'Você foi banido, por favor, contate o proprietário para saber mais',
					ownerG: '[❗] NANAMMY ? Este é um recurso especial para o NANAMMU  ❌',
					ownerB: '[❗] NANAMMY ? Este é um recurso especial para o NANAMMY  ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de NANAMMY . Registre-se para fazer amizade com o bot SIMIH  por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar SIMIH |15\n\n──「 SIMIH BOT 」──`,
			...
			const premium = ["556692272931@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("Ninguém xinga aqui, presta atenção e respeita teu próximo")
	}
	
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("foda que o corno é o único fiel na relação")
	}
		...
		if (messagesC.includes("sextou")){
			client.updatePresence(from, Presence.composing)
   reply("com S de cerveja^^")
 }
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS SIMIH BOT 🐊* 」\n\`\`\``
					no = 0
...
// recursos anime, random @simih  domina irmão
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "SIMIH ", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Nanammy  ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMIN SIMIH BOT - ADM NANAMMY  🇧🇷* 」\n*│+ wa.me/556692272931*╰──────*「 *ADM NANAMMY * 」*────*\n\n*_SE QUER SER ADMIN DO BOT SIMIH BOT_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia bebês ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/nSnRCIK.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde negada 😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do ADM NANAMMY :*\n\n tem não'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/meZwqA5.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* ADM NANAMMY \n*YOUTUBE:* off\n*WPP:* wa.me/+556692272931\n*INSTA:* sai\n\n\nEspero que tenham gostado do bot 🐊⚜️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
